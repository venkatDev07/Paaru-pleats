  ...
  return (
    <div className="h-full flex flex-col bg-primary text-cream">
      <div className="px-6 py-6 border-b border-cream/10">
        <span className="font-heading text-xl font-semibold text-cream hover:animate-pulse cursor-pointer" onClick={() => navigate('/')}>
          Saree<span className="text-accent">Pleating</span>
        </span>
        <p className="text-xs text-cream/50 mt-0.5 tracking-wide">Admin Panel</p>
      </div>
      {/* hover */}
      <nav className="flex-1 px-4 py-6 space-y-1.5 cursor-pointer">
        {links.map((link) => (
          <NavLink
            key={link.path}
            to={link.path}
